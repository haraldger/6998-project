...
def tetris():
    ...
    replay_buffer = ReplayBuffer(capacity=REPLAY_MEMORY, dims=DIMS)
    agent = SwinAgent("tetris", q_network, target_network, epsilon_scheduler, replay_buffer, num_actions=NUM_ACTIONS,
    ...
    for episode in range(EPISODES):
        ...
        if episode % 1000 == 0 and episode != 0:
            print(episode)
            plt.imsave('test_frame.png', np.moveaxis(next_state, 2, 0))
        total_reward += reward
...
def process_state(state):
    state = resize(state, (DIMS[1], DIMS[2]))
    state = np.moveaxis(state, 2, 0)
    return state


def get_model(image_size=(DIMS[1],DIMS[2]), patch_size=4, in_channels=3,
